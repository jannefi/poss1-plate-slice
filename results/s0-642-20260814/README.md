...
> **Correction (2026-09-09): the XE296 tile in that list,
> `tile_RA24.686_DECp33.529`, is defective after all** — by a different
> mechanism than the ~11″ fiducial divergence this section is about, and one
> the check above cannot see. The "median separation to Gaia" quoted here is
> the refit's residual on its own tie points; on that tile the fit had only 312
> tie points and extrapolates wrongly where it has none, so its 291 rows are
> largely catalogued stars displaced by a coherent ~7″. See **Known defect**
> below. The five XE074 tiles stand.
...
**The data files are unchanged.** This release cannot be regenerated byte for
byte (its tile tree is gone), so the defect is documented here rather than
patched; anyone who wants the rows out can drop them by `tile_id`.

**What is wrong.** The per-tile Gaia refit ("The coordinates carry a Gaia
refit", above) is a degree-2 polynomial fitted to detections matched to Gaia
within 5″. On `tile_RA24.686_DECp33.529` (plate XE296, a corner tile) the raw
plate solution is poor enough that the bootstrap found only **312** tie points
(survey median ~3,100; 1st percentile 928), and the fit — σ 0.48″ on those
points, reported `ok` — extrapolates wrongly across the rest of the tile. Real
stars there land ~7″ from their true positions, clear the 5″ vetoes, and
survive as catalogue rows.

**How much.** Every row of this catalogue was matched against its own tile's
plate-epoch Gaia neighbourhood, with a 60″-shifted control for chance:

| | rows | Gaia star at 3–8″ | chance | offset vector (row − star) |
|---|---:|---:|---:|---|
| `tile_RA24.686_DECp33.529` (XE296) | **291** | **52%** | 7% | **(−4.1″, −6.1″)**, coherent |
| `tile_RA63.809_DECp57.375` (XE084) | 46 | 91% | 41% | (+4.9″, +3.2″), coherent |
| **whole catalogue** | 122,820 | **5.8%** (7,126) | **12.5%** (15,297) | — |

**Survey-wide the catalogue sits well *below* chance for a nearby Gaia star**
— vetoed candidates avoid stars, as they should — so this is a local defect,
not a population: about 130 displaced rows on the XE296 tile and about 20 on
the XE084 tile (σ 0.88″, a dense field), ~0.1% of the catalogue. The
hot-tile plates (XE074, XE105, XE516, XE049) do not show it; their rows are
below chance for Gaia neighbours.

**Why the release audit missed it.** `verification_s0_gaia_invariant.csv.gz`
tests for Gaia stars *within 5″* — exactly what a 7″ displacement evades —
and the 2026-08-13 audit cleared this tile on the refit's own tie-point
residual, which is blind to extrapolation (see the correction above). The
check that finds it is the one used here: Gaia neighbours at 3–8″ against a
shifted control, per tile.

**What to do.** Filter `stage_S0.csv` on `tile_id` ∉ {`tile_RA24.686_DECp33.529`,
`tile_RA63.809_DECp57.375`} (337 rows) if you want the defect out; the
catalogue identity above still refers to the file as shipped. The pipeline
gains a guard against this failure mode (tie-point floor, σ ceiling with a
degree-1 fallback, and a post-fit self-check on the survivors' Gaia
neighbours, with quarantine of tiles that fail it); the companion build's
