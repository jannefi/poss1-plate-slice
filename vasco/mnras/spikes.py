...
def fetch_bright_ps1(
    ...
    (same pattern as fetch_ps1_neighbourhood in external_fetch_online.py).
    The live MAST call measured at ~31s/tile -- the dominant cost of
    step4/5 on a from-scratch tile -- so the local cache is strongly
    preferred whenever available.
    """
    if os.getenv("VASCO_PS1_CACHE"):
        from vasco.local_cache_query import query_ps1_bright_stars
        cached = query_ps1_bright_stars(ra_deg, dec_deg, radius_arcmin,
                                        rmag_max, mindetections)
